...
public class NearStadium { //DB데이터 중 제일 가까운 스타디움 가져오기
    ...
    public static List<StadiumDTO> findNearestStadiums(List<StadiumDTO> stdList,String x, String y, int count) {
        ...
        return stdList.subList(0, Math.min(count, stdList.size()));//구장 리스트 중 x,y값을 받아와서 count 만큼 가까운 구장 리스트빼는 메소드
    }
}
